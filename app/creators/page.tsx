...
export default function CreatorsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(usersCol, (snapshot) => {
      if (!snapshot.empty) {
        ...
        // 只過濾出「創作者」
        const creatorUsers = data.filter(u => u.role === '創作者');
        
        // 將基本資料與豐富的模擬 Media Kit 進行合併 (直到他們自己去後台編輯)
        const mappedCreators: CreatorDetail[] = creatorUsers.map((u, index) => {
          const enrich = ENRICH_DATA[index % ENRICH_DATA.length];
          const isFounder = index < 50; // 前 50 名加入的標記為創始會員
          ...
          return {
            ...
            // 巧妙地將創始會員加進 tags 中，讓 CreatorCard 可以直接顯示
            tags: isFounder ? ['👑 創始會員', ...enrich.tags] : enrich.tags,
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      {/* 🚀 Growth Hacking Banner: 創始會員招募進度條 */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-32 text-slate-400">
            ...
            <p className="font-medium tracking-widest uppercase text-xs">正在同步創作者資料...</p>
          </div>
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
            ...
            <p className="text-slate-500">系統中尚未有符合此條件的用戶。</p>
            <button 
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 bg-slate-900/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-4xl sm:rounded-3xl shadow-2xl overflow-y-auto flex flex-col animate-in slide-in-from-bottom-5 duration-300 relative">
            ...
            <div className="pt-16 px-6 sm:px-10 pb-8 flex-grow bg-slate-50/50">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start mb-8 gap-6">
                ...
                <div className="flex gap-3 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
                  ...
                  <div className="flex-1 sm:flex-none text-center p-4 bg-white rounded-2xl border border-slate-100 shadow-sm min-w-[100px]">
                    <p className="text-xs font-bold text-slate-400 mb-1 tracking-wider uppercase">互動率</p>
                    <p className="text-2xl font-black text-green-500">{selectedCreator.engagement}%</p>
                  </div>
                  <div className="flex-1 sm:flex-none text-center p-4 bg-white rounded-2xl border border-slate-100 shadow-sm min-w-[100px]">
                    <p className="text-xs font-bold text-slate-400 mb-1 tracking-wider uppercase">完成案件</p>
                    <p className="text-2xl font-black text-indigo-600">{selectedCreator.completedJobs}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 sm:p-6 border-t border-slate-200 bg-white sticky bottom-0 flex justify-between items-center gap-4 z-20">
               ...
               <button 
                 className="flex-1 sm:flex-none w-full sm:w-auto px-8 py-3.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 active:scale-95 transition-all"
